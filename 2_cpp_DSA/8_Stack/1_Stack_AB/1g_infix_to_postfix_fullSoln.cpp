...
// Helper function to find the in-Stack precedence of an operator
int inPre(char ch)
{
    if (ch == '+' || ch == '-')
    {
...
// Helper function to find the out-Stack precedence of an operator
int outPre (char ch)
{
    if (ch == '+' || ch == '-')
    {
...
char *InToPost(char *infix)
{
    ...
    while (infix[i] != '\0')
    {
        ...
        {
            if (outPre(infix[i]) > inPre(top->data)) //  star step
            {
                push(infix[i]);
                i++;
            }
...
int main()
{
    char infix[] = "((a+b)*c)-d^e^f"; // NOTE : a pointer is initialized with string value
